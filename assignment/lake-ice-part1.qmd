...
```{r}
library(readxl)
df <- read_excel(file_name, sheet = "data")
...
```{r}
sunapee <- filter(df, lake_name == "Sunapee")
...
```{r}
sunapee <- mutate(sunapee, ice_off_doy = as.integer(ice_off_doy))
sunapee |>
  ggplot(aes(x = year, y = ice_off_doy)) +
  geom_point() +
  geom_line() +
  theme_classic() +
  labs(x = "Year", y = "Ice Off Day", title = "Lake Sunapee Ice Off Days 1869-2025") 
...
Negative! The ice is melting earlier and earlier as the years progress, so the day of year value is descreasing as the years increase; it's an inverse relantionship.
...
Negative and it matches my expectations, it represents ice off starting earlier by one day each year. 
...
The r-squared value for the Sunapee regression is 0.2055 which doesn't  indicate a very strong relation, but might be okay-ish for ecology.
...
The data is slightly left-skewed; it has more negative  outliers than positive outliers, however, it still follows a bell-shaped curve with its peak near zero. I would say its generally meeting the normal distribution assumption.
...
sunapee |>
  ggplot(aes(resid)) + 
  geom_histogram(color = 'purple4', fill = 'purple', binwidth = 10) +
  theme_bw() +
  labs(x = "Residual Value", y = "Count", title = "Sunapee Lake Linear Model Residuals ")
...
70 years, if it changes by 0.1 days every year. I think that ice melting one week earlier is a significant environmental change, impacting any phenological timeline like crops, transport, hibernation, or dormancy. However, it might be happening slow enough that these processes can adjust. 
...
The model confirms the existence and direction of the change, which is useful and important, but the skewed distribution of the residuals and low r-squared value indicate that the linear model might not best reflect the pattern—so I don't think it can be used with any precision. 
...
sunapee |>
  ...
  theme_bw() +
  labs(x = "Year", y = "Ice Off Day of Year", title = "Lake Sunapee Ice Off Day 1869-2025")
...
```{r}
y <- (fit$coefficients[2] * 2040) + fit$coefficients[1]
#102.66

low <- quantile(sunapee$resid, 0.05) #-19.43
high <- quantile(sunapee$resid, 0.95) # 14.15

c(y + low, y + high)

```
The ice off day of year prediction for 2040 is 103 with a 34 day confidence interval (83-117). 
...
set1 <- filter(sunapee, year < 1970)
  
set2 <- filter(sunapee, year >= 1970)

fit1 <- lm(data = set1, ice_off_doy ~ year)
  
fit2 <- lm(data = set2, ice_off_doy ~ year)

sunapee |>
  ggplot(aes(x = year, y = ice_off_doy)) +
  geom_point()+
  geom_line()+
  geom_abline(intercept = fit1$coefficients[1], slope = fit1$coefficients[2], color = 'orange') +
  geom_abline(intercept = fit2$coefficients[1], slope = fit2$coefficients[2], color = 'red') +
  theme_classic() +
  labs(x = "Year", y = "Ice Off Day", title = "Ice Off Day over Time")

fit1$coefficients[2]
fit2$coefficients[2]

```
The linear model fitted to the later section (years 1970 and after) has a slope of a greater negative magnitude than the slope for the years before, it also has a greater magnitude than the slope calculated for the entire dataset. So, the rate of change of the slope itself is negative, the trend is accelerating. 
